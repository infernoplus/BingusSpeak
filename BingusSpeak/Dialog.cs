...
namespace BingusSpeak
{
    public class Dialog
    {
        ...
        public class DialogInfoRecord
        {
            ...
            public bool parent; // marked true if this line was used to split out more lines. this is used to prevent splitting a line more than once
            ...
            public DialogInfoRecord Split(ESM esm, JobInfo jobInfo)
            {
                string newText = text.Replace("%class", jobInfo.name, StringComparison.OrdinalIgnoreCase);

                DialogInfoRecord info = new(trueId, type, speaker, jobInfo.id, faction, cell, rank, race, sex, playerFaction, disposition, playerRank, filters, text, mp3, script);
                info.replacement = newText;

                foreach (CharacterContent content in used)
                {
                    if (content.job == jobInfo.id) { info.used.Add(content); }
                }

                return info;
            }

            public DialogInfoRecord Split(ESM esm, FactionInfo factionInfo)
            {
                string newText = text.Replace("%faction", factionInfo.name, StringComparison.OrdinalIgnoreCase);

                DialogInfoRecord info = new(trueId, type, speaker, job, factionInfo.id, cell, rank, race, sex, playerFaction, disposition, playerRank, filters, text, mp3, script);
                info.replacement = newText;
                
                foreach(CharacterContent content in used)
                {
                    if(content.faction == factionInfo.id) { info.used.Add(content); }
                }

                return info;
            }

            public DialogInfoRecord Split(ESM esm, FactionInfo factionInfo, int factionRank)
            {
                string rankName = factionInfo.GetRankName(factionRank);
                string newText = text
                    .Replace("%faction", factionInfo.name, StringComparison.OrdinalIgnoreCase)
                    .Replace("%rank", rankName, StringComparison.OrdinalIgnoreCase);

                DialogInfoRecord info = new(trueId, type, speaker, job, factionInfo.id, cell, factionRank, race, sex, playerFaction, disposition, playerRank, filters, text, mp3, script);
                info.replacement = newText;
            ...
            public DialogInfoRecord SplitPc(ESM esm, CharacterContent.Race raceVal)
            {
            ...
            public DialogInfoRecord SplitPc(ESM esm, MainWindow.PlayerJob jobVal)
            {
            ...
            public DialogInfoRecord SplitPc(ESM esm, FactionInfo factionInfo, int factionRank)
            {
                string rankName = factionInfo.GetRankName(factionRank);
                string nextRankName = factionInfo.GetRankName(factionRank+1);
                string newText = text
                    .Replace("%faction", factionInfo.name, StringComparison.OrdinalIgnoreCase)
            ...
            public enum Variable { Class, Name, PcClass, PcCrimeLevel, PcName, PcRace, Race, Cell, Faction, NextPcRank, PcNextRank, PcRank, Rank }
        ...
        public class DialogFilter
        {
            ...
            /* Used by dialog splitting to inject custom filters */
